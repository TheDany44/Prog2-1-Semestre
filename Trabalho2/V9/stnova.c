...
unsigned long sond_q(int pos, int tentativas, int tamanho);
...
int sondagem_fazer_origem(estrutura *st, char* cidade){
    ...
    int hash_pos,tentativas=1,s_pos;
    
    hash_pos=hash(cidade,st->capacidade);

    if(st->estado_orig[hash_pos]==NULL){
        return hash_pos;
    }

    int *control;
    control=calloc(st->capacidade,sizeof(int));
    if(control==NULL){return -1;}

    while(1){
        s_pos=sond_q(hash_pos,tentativas,st->capacidade);

        if(st->estado_orig[s_pos]==NULL){
            free(control);
            return s_pos;
        }
        if(control[s_pos]==1){
...
tab_destino *colocar_origem(estrutura *st,no_grafo *no){
    ...
    if(st->tamanho>=st->capacidade || st->capacidade==0){return NULL;}
...
int sondagem_fazer_destino(tab_destino *tab, char* cidade){
    ...
    hash_pos=hash(cidade,tab->capacidade);

    if(tab->estado_dest[hash_pos]==NULL){
        return hash_pos;
    }
    if(strcmp(tab->estado_dest[hash_pos],cidade)==0){
        return hash_pos;
    }

    int *control;
    control=calloc(tab->capacidade,sizeof(int));
    if(control==NULL){return -1;}


    while(1){
        s_pos=sond_q(hash_pos,tentativas,tab->capacidade);
        ...
        if(tab->estado_dest[s_pos]==NULL){
            free(control);
            return s_pos;
        }
        if(strcmp(tab->estado_dest[s_pos],cidade)==0){
            free(control);
            return hash_pos;
        }
...
int colocar_voo(tab_destino *tab_dest,aresta_grafo *aresta){
    ...
    if(tab_dest->elem[pos]->preco>aresta->preco){
