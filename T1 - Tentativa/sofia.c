...
void quickSortIter(colecao *c,int left,int right){
	int i, k;
	planta *ptemp;
	...
	if(strcmp(c->tipo_ordem,"id")==0){ 
		...
		if(right-left+1<20){
			...
			c->plantas[right]=ptemp;
			quickSortIter(c,left,i-1);
			quickSortIter(c,i+1,right);
		}
	}
	else if(strcmp(c->tipo_ordem,"nome")==0){ 
