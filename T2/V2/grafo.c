...
/*int atualiza_no(no_grafo* no,no_grafo* prev,aresta_grafo* voo){
    if(no==NULL || prev==NULL || voo==NULL){return 0;}

    time_t partida, chegada, dia;
    
    partida=mktime(&voo->partida);
    chegada=mktime(&voo->chegada); 
    printf("%d ------ ",prev->dataatualizada->tm_isdst);
    dia=mktime(prev->dataatualizada);
    printf("%d ------ ",prev->dataatualizada->tm_isdst);

    double new_peso;

    new_peso=difftime(partida,dia);

    if(new_peso<0){
        return 0;
    }

    new_peso+=difftime(chegada,partida);

    if(new_peso>no->p_acumulado){
        return 0;
    }
    
    no->p_acumulado=new_peso;
    no->anterior=prev;
    dia+=new_peso;
    no->dataatualizada=gmtime(&dia);
    
    return 1;
    
}*/

int heap_existe(no_grafo* no,heap *h){
    if(no==NULL || h==NULL){
        return 0;
    }
    int i;
...
int dijstra_inicia(heap *h,grafo *g,no_grafo *origem){
    if(h==NULL || g==NULL || origem==NULL){return 1;}
    int i;
    for(i=0;i<g->tamanho;i++){
        if(origem==g->nos[i]){
        ...
        if(!heap_insere(h,g->nos[i],g->nos[i]->p_acumulado)){return 1;}
    }
    return 0;
}

void atualiza_heap(no_grafo* no,heap *h, heap *aux){
    if(no==NULL || h==NULL || aux==NULL){return;}

    if(!heap_existe(no,h)){return;}
    no_grafo *temp;
    temp=heap_remove(h);
    while(temp!=no){
...
{
    /*if(g==NULL || origem==NULL || destino==NULL){return NULL;}

    no_grafo *origem_no, *destino_no;

    origem_no=encontra_no(g,origem); //verificar se origem existe
    if(origem_no == NULL){return NULL;}

    destino_no=encontra_no(g,destino);  //verificar se destino existe
    ...
    if(aux==NULL){heap_apaga(h);return NULL;}


    int i;

    origem_no->dataatualizada=&partida;
    origem_no->anterior=NULL;
    
    for(i=0;i<g->tamanho;i++){
        if(origem_no==g->nos[i]){
            origem_no->p_acumulado=0;
        }
        else{
            g->nos[i]->p_acumulado=__DBL_MAX__;
        }
    ...
    while(h->tamanho!=0){
        ...
        if(atual->p_acumulado==__DBL_MAX__){
        ...
        for(i=0;i<atual->tamanho;i++){
            ...
            if(heap_existe(atual->arestas[i]->destino,h)){
                if(atualiza_no(atual->arestas[i]->destino,atual,atual->arestas[i])){
                    atualiza_heap(atual->arestas[i]->destino,h,aux);
                }
            }
    ...
    no_grafo* temp;
    *n=0;
    for(temp=destino_no;temp->anterior!=NULL;temp=temp->anterior){
        (*n)++;
    }
    no_grafo **vetor;
    ...
    for(i=*n-1;i>=0;i--){
        vetor[i]=destino_no;
        destino_no=destino_no->anterior;
    }
...
no_grafo **menos_transbordos(grafo *g, char *origem, char *destino, data partida, int *n)
{
    ...
    h=heap_nova(g->tamanho);
    if(h==NULL){return NULL;}
    aux=heap_nova(g->tamanho);
    if(aux==NULL){return NULL;}

    if(dijstra_inicia(h,g,origem_no)){return NULL;}
    ...
    while(h->tamanho!=0){
        ...
        if(no==destino_no){break;}
        if(no->p_acumulado==__DBL_MAX__){return NULL;}
        peso++;
        for(i=0;i<no->tamanho;i++){
            if(no->arestas[i]->destino->p_acumulado>peso){
                no->arestas[i]->destino->p_acumulado=peso;
                atualiza_heap(no->arestas[i]->destino,h,aux);
                no->arestas[i]->destino->anterior=no;
            }
        }
    ...
    *n=0;
    ...
    for(no=destino_no;no!=NULL;no=no->anterior){
        (*n)++;
    }
    no_grafo**vetor;
    vetor=malloc((*n)*sizeof(no_grafo*));
