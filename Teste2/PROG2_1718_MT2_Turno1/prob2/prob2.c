...
{
    if(g==NULL || origem<0 || destino<0 || cache==NULL){return -1;}
    ...
    const char *valor;
    int distancia;
    valor=tabela_valor(cache,chave);
    if(valor!=NULL){distancia=atoi(valor);return distancia;}

    int *caminho;
    caminho=grafo_bfs(g,origem,destino,&distancia);
    if(caminho==NULL){return -1;}
    distancia--;
    free(caminho);

    char inserir[TAMANHO_VALOR];
    sprintf(inserir,"%d",distancia);
    if(tabela_adiciona(cache,chave,inserir)==TABDISPERSAO_ERRO){return -1;}
