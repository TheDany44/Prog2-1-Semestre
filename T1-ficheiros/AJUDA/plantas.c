...
int planta_insere(colecao *c, planta *p)
{
    ...
    for(int i=0;i<c->tamanho;i++)
    {
        if(strcmp(p->ID, c->plantas[i]->ID)==0)
        {
            ...
            for (int n=0;n<p->n_alcunhas;n++)
            {
                for(int l=0;l<c->plantas[i]->n_alcunhas; l++)
                {
                    if (strcmp(c->plantas[i]->alcunhas[l],p->alcunhas[n])==0) break;
                    else 
                    {
                        m++;
                        c->plantas[i]->alcunhas[m]= malloc(strlen(p->alcunhas[l]+1)*sizeof(char));
                        strcpy(c->plantas[i]->alcunhas[m],p->alcunhas[l]);
                    }
                }
            }
            c->plantas[i]->n_alcunhas= m;
            return 1;
        }
    }
    ...
    {
        for(int k=0; k<c->tamanho-1;k++)
        {
            if(strcmp(p->nome_cientifico,c->plantas[k]->nome_cientifico) < 0) 
            {
                pos=k;
                break;
            }    
    ...
    else if (strcmp(c->tipo_ordem, "id")==0) 
    {
        for(int k=0; k<c->tamanho-1;k++)
        {
            if(strcmp(p->ID,c->plantas[k]->ID) < 0) 
            {
                pos=k;
                break;
            }  
    ...
    for (int n=c->tamanho-2; n >= pos; n--)
    {
        c->plantas[n+1]=c->plantas[n];
    } 
    c->plantas[pos] = p; 
    return 0; 
}
